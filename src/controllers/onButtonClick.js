...
export default async button => {
  ...
  if (guildDB.applications_channel === undefined) {
    await button.reply.send('Не установлен канал для отправки заявок, обратитесь к администрации сервера')
    return
  }
  ...
  if(eventMember) {
    ...
    if (application === null) {
      await button.reply.send(`У вас ещё не заполнена заявка, заполните её в канале <#${guildDB.new_app_channel}>`, true)
      return
    }
  ...
  await Promise.all(event.members.map(async (evMember, index) => {
    const memberApplication = await ApplicationModel.findOneByID(evMember.id)
    embedMembers.addField(`${index+1}.`, `|- <@${memberApplication.id}> (${memberApplication.level} уровень)\n|- ${memberApplication.link}`)})
  )
  const messageApps = await channelApps.messages.fetch(event.message_apps_id)
  if(messageApps === undefined) await button.reply.send('Ошибка. Обратитесь к администрации сервера.')
